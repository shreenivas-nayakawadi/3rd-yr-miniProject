    ...
    (set) => ({
      ...
      createBudget: async (userId, budgetData) => {
        ...
        try {
          ...
          set({
            error: error.response?.data?.message || "Error creating budget",
            isLoading: false,
          });
          throw error;
        }
        ...
        try {
          ...
          set((state) => ({
            budgets: state.budgets.map((budget) =>
              budget.budget_id === budgetId ? response.data : budget
            ),
            isLoading: false,
          }));
        } catch (error) {
          set({
            error: error.response?.data?.message || "Error updating budget",
            isLoading: false,
          });
          throw error;
        }
      ...
      deleteBudget: async (userId, budgetId) => {
        set({ isLoading: true, error: null });
        try {
          await axios.delete(`${API_URL}/delete/${userId}/${budgetId}`);
          set((state) => ({
